...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        if (session?.user) {
          setTimeout(() => {
            checkUserApprovalAndRole(session.user.id, session.user.email).finally(() => {
              setLoading(false);
            });
          }, 0);
        } else {
  ...
  const checkUserApprovalAndRole = async (userId: string, email?: string) => {
    try {
      ...
      if (roleCheckError) {
        ...
        // Fallback: treat super admin email as admin
        const isSuperAdmin = email === 'tokoanjar09@gmail.com';
        setIsAdmin(Boolean(hasAdminRole) || isSuperAdmin);
      }
    } catch (error) {
